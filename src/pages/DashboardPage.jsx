...
export default function DashboardPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const [visits, persons] = await Promise.all([
        visitsApi.getActive(),
        personsApi.getAll(),
      ]);
      setActive(visits);
      ...
      <div>
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            description="Las entradas registradas aparecerán aquí."
          />
        ) : (
...
function StatCard({ label, value, accent }) {
  return (
    <div className="ledger-card px-5 py-4">
      ...
      <p
        className={`text-3xl font-bold mt-1 ${accent === "active" ? "text-stamp-active" : "text-ink"}`}
      >
        {value}
      </p>
    </div>
